...
exports.setCell = exports.validateTable = exports.createTable = undefined;
...
var createRow = function createRow() {
    return _lodash2.default.times(9, _lodash2.default.constant(null));
};
var createTable = exports.createTable = function createTable() {
    return _lodash2.default.times(9, createRow);
};

// Testing
var createRow2 = function createRow2(x) {
    return _lodash2.default.map(_lodash2.default.range(9), function (y) {
        return x + '-' + y;
    });
};
var createTable2 = function createTable2() {
    return _lodash2.default.map(_lodash2.default.range(9), createRow2);
};

var getRow = function getRow(table, row) {
    return table[row];
};
var getColumn = function getColumn(table, columnIndex) {
    return _lodash2.default.map(table, function (row) {
        return row[columnIndex];
    });
};

var getArea = function getArea(table, x, y) {
    var rows = [table[3 * y], table[3 * y + 1], table[3 * y + 2]];
    return _lodash2.default.flatMap(rows, function (row) {
        return row.slice(3 * x, 3 * x + 3);
    });
};
var getAllAreas = function getAllAreas(table) {
    return (0, _lodash2.default)(3).range().flatMap(function (x) {
        return (0, _lodash2.default)(3).range().map(function (y) {
            return getArea(table, x, y);
        }).value();
    }).value();
};

var getMainDiag = function getMainDiag(table) {
    return _lodash2.default.map(table, function (row, index) {
        return row[index];
    });
};
var getBackDiag = function getBackDiag(table) {
    return _lodash2.default.map(table, function (row, index) {
        return row[8 - index];
    });
};

var getAllConstraints = function getAllConstraints(table) {
    var rows = table;
    var columns = _lodash2.default.range(9).map(_lodash2.default.partial(getColumn, table));
    var areas = getAllAreas(table);
    var diags = [getMainDiag(table), getBackDiag(table)];

    return [].concat(_toConsumableArray(rows), _toConsumableArray(columns), _toConsumableArray(areas), diags);
};

var allDistinct = function allDistinct(values) {
    var filteredValues = _lodash2.default.filter(values, function (x) {
        return typeof x === 'number';
    });
    return _lodash2.default.uniq(filteredValues).length === filteredValues.length;
};

var allDistinctList = function allDistinctList(constraintList) {
    return _lodash2.default.reduce(constraintList, function (acc, values) {
        return acc && allDistinct(values);
    }, true);
};

var validateTable = exports.validateTable = function validateTable(table) {
    return allDistinctList(getAllConstraints(table));
};

var setCell = exports.setCell = function setCell(table, rowIndex, columnIndex, value) {
    var newTable = _lodash2.default.cloneDeep(table);
...
/* WEBPACK VAR INJECTION */}.call(exports, __webpack_require__(59), __webpack_require__(301)(module)))
...
var _sudokuModel = __webpack_require__(114);

var _sudokuPresenter = __webpack_require__(302);

var t = (0, _sudokuModel.createTable)();
...
(0, _sudokuPresenter.renderTable)(t);
(0, _sudokuPresenter.renderTimer)();
...
exports.renderTimer = exports.renderTable = undefined;
...
var templateString = document.getElementsByClassName('js-table-template')[0].innerHTML;

var tableTemplate = _lodash2.default.template(templateString);

var selectCell = function selectCell(domNode) {
    _lodash2.default.each(document.getElementsByClassName('js-selected'), function (node) {
...
var addClickEvent = function addClickEvent(item) {
...
var addAllClickEvents = function addAllClickEvents() {
    return _lodash2.default.each(document.getElementsByClassName('js-sudoku-cell'), addClickEvent);
};

var insertValue = function insertValue(key) {
    var selectedNode = document.getElementsByClassName('js-selected')[0];
    if (selectedNode) {
        var dataset = selectedNode.dataset;
        var value = parseInt(key) || null;
        renderTable((0, _sudokuModel.setCell)(tableModel, dataset.row, dataset.column, value));
    }
};

var addKeyEvent = function addKeyEvent() {
...
var renderTable = exports.renderTable = function renderTable(table) {
    var isValid = (0, _sudokuModel.validateTable)(table);
    document.getElementsByClassName('js-table')[0].innerHTML = tableTemplate({ table: table, isValid: isValid });
...
var renderTimer = exports.renderTimer = function renderTimer() {
    var t = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : 0;

    document.getElementsByClassName('js-timer')[0].innerHTML = _lodash2.default.template('<%= t %> seconds')({ t: t });
    // `${t} seconds`
    _lodash2.default.delay(function () {
        return renderTimer(t + 1);
    }, 1000);
};
